...
const page = async ({ params }: Props) => {
    const { id } = await params;
    const course: { course: ICourse } = await courseService.handelGetCourseById(id);
    const { buyers, description, instructor_id, price, title, rating, _id } = course?.course
    return (
        <WrapperBody>
            ...
            <section className='w-full flex max-lg:flex-col gap-10'>
                <div className="flex-1">
                    ...
                    <div className="flex-col w-full flex gap-2 mt-5">
                        <div className="px-[2.4vw]">
                            <h3 className='text-4xl font-semibold'> {instructor_id?.name} </h3>
                ...
                <div className="flex-[0.5]">
                    <h4 > <span className='text-[4.2vw]'> {formatNumberEU(price)} </span> USD </h4>
                    ...
                    <div className="flex gap-4 mt-4">
                        <BuyCourseButton courseDescription={course?.course?.description} courseId={_id!} price={price || ""} imgs={[]} courseTitle={title} variant='black' />
